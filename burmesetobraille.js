...
function convert(burmese) {
  ...
  for (let x = 0; x < burmese.length; x++) {
    ...
    if (element == "င" && burmese[x + 1] == "်" && burmese[x + 2] == "္") {
      ...
      contain_non_burmese = true;
      non_burmese_indexes.push(x);
    }
  }
  if (contain_non_burmese) {
    const is_plural = non_burmese_indexes.length > 1;
    var message =
      "This text contains" +
      (is_plural ? "" : " a") +
      " non-burmese or non-unicode character" +
      (is_plural ? "s" : "") +
      " at index" +
      (is_plural ? "es" : "") +
      " " +
      non_burmese_indexes +
      ".\n " +
      (is_plural ? "These" : "This") +
      " will not be included in the braille conversion.";
